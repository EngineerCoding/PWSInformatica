...
public final class JSONArray extends JSONParser {
	...
	public JSONArray(final Tokenizer tokenizer) {
		...
		if(tokenizer.isNext(Constants.ArrayStartingChar)) {
			do {
			...
			if(tokenizer.isNext(Constants.ArrayEndingChar))
				return;

			throw new SyntaxException(Constants.expectedCharFormat, Constants.ArrayEndingChar);
		}
		throw new SyntaxException(Constants.expectedCharFormat, Constants.ArrayStartingChar);
	}
	...
	public boolean has(final int key) {
		if(key >= 0 && key < storage.size())
            return true;
        throw new JSONException(Constants.expectedExistingKeyFormat, key, Constants.typeJSONArray);
	}
