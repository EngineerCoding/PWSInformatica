	...
	private void getCommits () {
		try {
			...
			for (int i = 0; i < values.getSize(); i++) {
				...
				writer.write(String.format(FORMAT_LINE, object.getString(DATE), countDiff(content, MODE.ADD), countDiff(content, MODE.MIN), object.getString(HASH), object.getString(MESSAGE)));
				//writer.write(System.lineSeparator());
				System.out.println(String.format("%s +%04d -%04d", object.getString(DATE), countDiff(content, MODE.ADD), countDiff(content, MODE.MIN)));
			}
			writer.flush();
